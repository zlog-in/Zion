...
// generate signature for the header hash?
...
func (s *SignerImpl) SealBeforeCommit(h *types.Header) error {
	sigHash := s.SigHash(h)              // generate hash for header
	seal, err := s.Sign(sigHash.Bytes()) // generate the signature by signer
	if err != nil {
		return errInvalidSignature
	}

	if len(seal)%types.HotstuffExtraSeal != 0 { // why isn't equal to zero
		return errInvalidSignature
	}
	...
	extra.Seal = seal                         // add seal(signature) to extra
	payload, err := rlp.EncodeToBytes(&extra) // encode extra as payload
	if err != nil {
		return err
	}
	h.Extra = append(h.Extra[:types.HotstuffExtraVanity], payload...) // add payload to header's extra field
	return nil
}
...
func (s *SignerImpl) SealAfterCommit(h *types.Header, committedSeals [][]byte) error {
	if len(committedSeals) == 0 { // what is the real length of committedSeals?
	...
	extra, err := types.ExtractHotstuffExtra(h) // extra is the type HotstuffExtra
	if err != nil {
		return err
	}
...
func (s *SignerImpl) VerifyHeader(header *types.Header, valSet hotstuff.ValidatorSet, seal bool) (*types.HotstuffExtra, error) {
	...
	// Signer should be in the validator set of previous block's extraData. previous or current?
...
func (s *SignerImpl) VerifyQC(qc *hotstuff.QuorumCert, valSet hotstuff.ValidatorSet) error {
	...
	extra, err := types.ExtractHotstuffExtraPayload(qc.Extra) // qc.Extra is []byte, but extra is HotstuffExtra
	if err != nil {
		return err
	}
...
func (s *SignerImpl) CheckQCParticipant(qc *hotstuff.QuorumCert, signer common.Address) error {
	// this signer may not be the proposer, may be a validator
	if qc.View.Height.Uint64() == 0 {
		return nil
	}
	...
	// only checker proposer?
	if signer == qc.Proposer && signer == proposer { // what is the difference between qc.Proposer and proposer
		return nil
	}
...
// check if a signature is generated by one of the validators
...
func (s *SignerImpl) VerifyHash(valSet hotstuff.ValidatorSet, hash common.Hash, sig []byte) error {
	...
	if _, val := valSet.GetByAddress(signer); val == nil { // val == nil means signer is not in the validators set?
...
func checkValidatorQuorum(committers []common.Address, valSet hotstuff.ValidatorSet) error {
	...
	// The length of validSeal should be larger than number of faulty node + 1 ??? >= 3f+1, Q = N - F
...
// verify sig and return address
func getSignatureAddress(data []byte, sig []byte) (common.Address, error) {
	// 1. Keccak data
	hashData := crypto.Keccak256(data) // vote means signing a hash value which is generated from header hash
